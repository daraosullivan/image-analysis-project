...
public class BinaryImageUtil {
	 ...
	 * specified threshold being used to determine which pixels become black and
	 * which become white.
	 * 
	 * The algorithm works as follows: - Convert the entire image to greyscale, so
	 * each pixel is now a level of "greyness" between 0 and 255. - Determine if
	 * each pixel is above or below the level of "greyness" in the threshold. If
	 * it's above, make the pixel black, if it's below make the pixel white.
	 * 
	 * This is basically bringing the image to two extremes. Experimentation may
	 * need to be done with the provided threshold.
	 * 
	...
	public static BufferedImage convert(final BufferedImage toConvert, final int threshold) {
		...
		final BufferedImage convertedImage = new BufferedImage(toConvert.getWidth(), toConvert.getHeight(),
